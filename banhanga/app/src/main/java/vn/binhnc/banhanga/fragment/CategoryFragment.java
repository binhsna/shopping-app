...
import vn.binhnc.banhanga.adapter.RecyclerViewMealByCategory;
import vn.binhnc.banhanga.database.LocalDatabase;
import vn.binhnc.banhanga.databinding.FragmentCategoryBinding;
    ...
    private void getData(int page) {
        compositeDisposable.add(apiBanHang.getSanPhamOnPage(page, loai)
                ...
                .subscribe(
                        ...
                        }, throwable -> Toast.makeText(getActivity(), "Không kết nối được với server" + throwable.getMessage(), Toast.LENGTH_LONG).show()
                ));
    }
    ...
    private void UpdateFavorite(int sp_id, String action) {
        compositeDisposable.add(apiBanHang.updateFavorite(sp_id, Utils.user_current.getId(), action)
                ...
                .subscribe(
                        ...
                        }, throwable -> Toast.makeText(getActivity(), "Lỗi: " + throwable.getMessage(), Toast.LENGTH_LONG).show()
                ));
    }
